...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | tree에는 배열처럼 하나뿐인 읽기 순서가 없으므로, 부모와 자식 중 무엇을 먼저 처리할지 목적에 맞게 정하기 위해 배운다. |
| 판단 근거 | NIST는 tree traversal을 tree의 node를 체계적으로 방문하는 과정으로 정의하며 preorder·inorder·postorder 같은 순서를 구분한다. |
| 실제로 어디에 쓰이나 | 전체 검색·출력·복사·직렬화·subtree 집계와, BST의 정렬 순회처럼 모든 node를 특정 의존 순서로 처리할 때 쓴다. |
| 기억할 장면 | 부모를 먼저, 중간에, 마지막에 볼지 또는 같은 level부터 볼지를 고른다. |
