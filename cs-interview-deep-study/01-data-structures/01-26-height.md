...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | node에서 가장 먼 leaf까지 남은 경로가 tree 연산의 최악 탐색 단계와 균형 상태를 결정하기 때문에 배운다. |
| 판단 근거 | NIST는 node height를 그 node에서 descendant leaf까지 가장 긴 경로의 edge 수로 정의한다. leaf height는 `0`이다. |
| 실제로 어디에 쓰이나 | AVL의 balance factor, BST·균형 tree·heap의 시간 복잡도 설명, 재귀가 얼마나 깊어질 수 있는지 판단할 때 쓴다. |
| 기억할 장면 | height는 “나에서 가장 깊은 leaf까지 남은 계단 수”다. |
