...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | binary tree에서 현재 node를 두 subtree 사이에 방문하는 순서와, BST에서는 그 결과가 key 정렬 순서가 되는 이유를 이해하기 위해 배운다. |
| 판단 근거 | NIST는 inorder를 `left subtree → root → right subtree` 순서로 정의한다. BST의 왼쪽·오른쪽 순서 불변식과 결합하면 오름차순 방문이 된다. |
| 실제로 어디에 쓰이나 | BST의 정렬된 key 출력·iteration, 범위 안의 key 순회, 역순 inorder를 이용한 내림차순 처리에 쓴다. 일반 tree에 그대로 적용되는 순서는 아니다. |
| 기억할 장면 | 왼쪽을 다 보고, 나를 보고, 오른쪽을 본다. |
