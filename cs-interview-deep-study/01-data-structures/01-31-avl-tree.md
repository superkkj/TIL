...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | 각 node의 좌우 height 차이를 엄격히 제한하고 LL·RR·LR·RL 회전으로 복구하는 균형 tree의 핵심을 익히기 위해 배운다. |
| 판단 근거 | NIST는 AVL Tree를 각 node의 두 subtree height 차이가 최대 1인 binary search tree로 정의한다. |
| 실제로 어디에 쓰이나 | 조회 경로를 짧게 유지하는 ordered search 구조를 설계하고, Red-Black Tree와 조회·갱신 비용의 절충을 비교할 때 쓰인다. Java 표준 컬렉션이 AVL을 사용한다고 단정하면 안 된다. |
| 기억할 장면 | 모든 node에서 왼쪽 높이와 오른쪽 높이의 차이가 `-1`, `0`, `1` 안에 있어야 한다. |
