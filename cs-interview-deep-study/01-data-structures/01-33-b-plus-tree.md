...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | 단건 탐색뿐 아니라 정렬된 범위를 연속으로 읽는 요구를 tree 탐색과 leaf 연결로 함께 해결하는 원리를 이해하기 위해 배운다. |
| 판단 근거 | NIST의 B+Tree 정의는 search key를 leaf에 두고 leaf를 순서대로 연결하는 구조를 설명한다. 이 구조가 첫 leaf를 찾은 뒤 다음 leaf로 이동하는 범위 순회를 가능하게 한다. |
| 실제로 어디에 쓰이나 | database index의 단건·범위·정렬 조회 원리를 공부하고 page 기반 ordered index를 설계할 때 쓴다. 정확한 page 형식과 payload 위치는 저장 엔진별 공식 문서로 확인한다. |
| 기억할 장면 | 내부 node는 표지판이고, 실제 항목은 연결된 leaf 페이지를 따라 순서대로 읽는다. |
