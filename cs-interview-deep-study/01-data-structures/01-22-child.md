...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | 한 node 아래로 이어지는 직접 관계를 알아야 subtree 구성, 아래 방향 순회, 삽입 위치를 정확히 설명할 수 있다. |
| 판단 근거 | NIST는 child를 edge 하나로 parent 아래에 직접 연결된 node로 정의한다. 모든 descendant가 곧바로 child인 것은 아니다. |
| 실제로 어디에 쓰이나 | binary tree의 `left`·`right`, 일반 tree의 children 목록, heap·BST·구문 tree의 아래 방향 탐색을 구현할 때 쓴다. |
| 기억할 장면 | child는 자손 전체가 아니라 바로 한 계단 아래에 있는 node다. |
