...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | 도착 순서가 아니라 값·마감시간·점수 같은 우선순위에 따라 “다음 하나”를 효율적으로 고르는 문제를 풀기 위해 배운다. |
| 판단 근거 | Oracle Java 17 `PriorityQueue`는 natural ordering이나 `Comparator`에 따라 head를 정하고, `offer`·`poll`은 O(log n), `peek`는 상수 시간이라고 명시한다. |
| 실제로 어디에 쓰이나 | 다음 실행 작업 선택, 가장 작은 값 반복 추출, 상위·하위 `k`개 유지처럼 매번 우선순위가 가장 높은 후보가 필요할 때 쓴다. |
| 기억할 장면 | 응급실에서는 먼저 도착한 사람보다 더 위급한 사람을 먼저 본다. |
