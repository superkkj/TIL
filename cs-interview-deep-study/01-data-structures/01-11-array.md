...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | index 직접 접근이 왜 빠르고, 중간 삽입·삭제에는 왜 원소 이동이 필요한지 이해하면 여러 상위 자료구조의 비용도 설명할 수 있다. |
| 판단 근거 | Java SE 17 JLS는 배열을 고정된 길이의 component 모음으로 정의하고, 유효 index를 `0`부터 `length - 1`까지로 정한다. |
| 실제로 어디에 쓰이나 | 고정 크기 데이터, 표·행렬, 버퍼, 그리고 배열 기반 List·heap·hash table 구현의 저장 공간으로 쓰인다. |
| 기억할 장면 | 번호가 붙은 사물함은 번호로 바로 열 수 있지만, 중간에 새 칸을 끼우려면 뒤 물건을 밀어야 한다. |
...
`logicalSize`는 **배열 전체 칸 중 현재 실제 데이터로 사용하는 칸의 개수**다.
Java 배열이 자동으로 제공하는 값이 아니라, 배열을 List처럼 사용하려는 개발자가
직접 만들고 관리하는 변수다.

```text
a.length = 6       // 배열을 만들 때 확보한 전체 칸 수
logicalSize = 4    // 그중 실제 값이 들어 있다고 보는 칸 수
...
주차장으로 비유하면 `a.length`는 **전체 주차 자리 수**, `logicalSize`는
**현재 주차된 차량 수**다. 주차장이 6칸이어도 차가 4대만 있다면 두 값은 각각
`6`과 `4`다.

따라서 값을 하나 삽입하면 `logicalSize++`로 실제 데이터 수를 1 늘리고, 값을 하나
삭제하면 `logicalSize--`로 1 줄여야 한다. 반면 `a.length`는 새 배열을 만들지 않는 한
바뀌지 않는다.
...
코드가 한눈에 안 들어오면 `System.arraycopy`의 다섯 자리를 먼저 다음처럼 읽으면 된다.
...
위 코드에서는 원본 배열과 대상 배열이 둘 다 `a`다. 즉, 다른 배열로 복사하는 것이
아니라 **같은 배열 안의 값들을 오른쪽으로 한 칸 미는 것**이다.

예를 들어 다음 배열의 `index 1` 자리에 `X`를 넣는다고 하자.
...
복사할 원소가 3개인 이유는 `logicalSize - index`, 즉 `4 - 1 = 3`이기 때문이다.
삽입할 자리인 1번부터 마지막 값인 3번까지 `[B, C, D]` 세 개를 옮겨야 한다.

각 줄이 실행될 때 배열은 다음처럼 바뀐다.

```text
처음
[A] [B] [C] [D] [ ] [ ]

1. System.arraycopy(...) 실행
...
한 문장으로 줄이면 **“넣을 자리부터 뒤의 값들을 오른쪽으로 밀고, 생긴 자리에 새 값을
넣은 다음, 사용 중인 원소 개수를 1 늘린다”**이다.

여기서 `[ ]`는 설명을 위해 빈 칸처럼 표시한 것이다. 실제 배열에서는 배열 타입에 따라
`null`, `0` 같은 기본값이 들어 있다.
