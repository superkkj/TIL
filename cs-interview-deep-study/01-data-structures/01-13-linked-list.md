...
- 구현 근거: [OpenJDK 17u LinkedList.java](https://github.com/openjdk/jdk17u/blob/master/src/java.base/share/classes/java/util/LinkedList.java), [OpenJDK 17u ArrayList.java](https://github.com/openjdk/jdk17u/blob/master/src/java.base/share/classes/java/util/ArrayList.java)
...
- Java 문법 근거: [JLS 4.12.5 필드 초기값](https://docs.oracle.com/javase/specs/jls/se17/html/jls-4.html#jls-4.12.5), [JLS 8.8.9 기본 생성자](https://docs.oracle.com/javase/specs/jls/se17/html/jls-8.html#jls-8.8.9), [JLS 15.26.1 대입 순서](https://docs.oracle.com/javase/specs/jls/se17/html/jls-15.html#jls-15.26.1)
...
`Node`는 값 하나와 앞뒤 Node로 가는 참조를 함께 담은 **기차 칸 하나**다.
Java의 공통 `Node` 클래스가 있는 것이 아니라, OpenJDK의 `LinkedList` 클래스
안에 별도로 선언된 내부 클래스다. HashMap의 Node와 이름만 같고 필드와 역할은
다르다.
...
오른쪽의 `new Node<>(value, head)`가 먼저 평가되고, 완성된 새 Node 참조가 왼쪽 `head`에
저장된다. 따라서 생성자에 전달되는 `head`는 바뀌기 전의 기존 head다. 그 뒤 `size++`로
Node 수를 1 늘린다. [근거: JLS 15.26.1]
