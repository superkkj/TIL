...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | 메모리 비교 횟수보다 page·block 접근 횟수가 중요한 저장장치에서, 한 node에 여러 key를 넣어 tree height를 낮추는 이유를 이해하기 위해 배운다. |
| 판단 근거 | NIST는 B-Tree를 여러 key와 child를 한 node에 저장하고 모든 leaf를 같은 level에 유지하는 균형 search tree로 정의한다. |
| 실제로 어디에 쓰이나 | page 단위로 읽는 저장장치와 database·파일 index 계열을 이해할 때 쓴다. 실제 node 크기와 key 배치는 사용하는 저장 엔진의 공식 문서를 확인해야 한다. |
| 기억할 장면 | 디스크 page 하나를 읽었을 때 이진 분기 하나가 아니라 여러 갈래 길 안내를 한꺼번에 얻는다. |
