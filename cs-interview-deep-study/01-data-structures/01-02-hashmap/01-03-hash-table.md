...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | 큰 key 공간을 작은 bucket 배열로 바꾸어 전체 순회 없이 찾는 원리와, 그 대가인 충돌·resize를 함께 이해하기 위해 배운다. |
| 판단 근거 | 위 NIST 정의는 hash table을 key를 table 위치로 연결하는 구조로 설명하고, Java `HashMap`은 이 원리를 사용하는 `Map` 구현의 공식 사례다. |
| 실제로 어디에 쓰이나 | key-value 사전, 집합의 원소 존재 확인, 빈도 집계, 메모리 안의 조회표를 구현하는 기반으로 쓰인다. |
| 기억할 장면 | 모든 서랍을 뒤지지 않고 key로 서랍 번호 후보를 먼저 계산한다. |
