...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | 충돌은 피해야 할 예외가 아니라 유한한 bucket을 쓰면 생길 수 있는 정상 상황이다. 이를 처리해야 조회 결과의 정확성과 성능을 지킬 수 있다. |
| 판단 근거 | NIST hash table 정의와 Java `HashMap` 계약은 hash만으로 key의 동일성을 확정하지 않으며, 같은 위치의 후보들 중 실제 key를 비교하는 구조를 전제로 한다. |
| 실제로 어디에 쓰이나 | 사용자 정의 key를 설계하고, `HashMap`·`HashSet`의 느린 조회나 한 bucket 쏠림을 분석하며, 충돌 해결 방식을 고를 때 쓰인다. |
| 기억할 장면 | 같은 사물함 번호를 받았으면 이름표인 `equals()`를 다시 확인한다. |
