...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | table을 너무 빽빽하게 써서 충돌을 늘리지 않으면서도 빈 공간을 과도하게 만들지 않는 시간·공간 절충을 이해하기 위해 배운다. |
| 판단 근거 | Oracle Java 17 `HashMap` 문서는 capacity와 load factor를 성능에 영향을 주는 핵심 값으로 설명하고 기본 load factor를 `0.75`로 둔다. |
| 실제로 어디에 쓰이나 | 대량 데이터를 넣을 `HashMap`의 초기 capacity를 잡고, resize 반복·메모리 여유·충돌 증가를 분석할 때 쓰인다. |
| 기억할 장면 | 주차장이 완전히 차기 전에 더 큰 주차장으로 옮겨 탐색 혼잡을 줄인다. |
