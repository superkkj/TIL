...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | 충돌 시 별도 Node를 연결하지 않고 배열 안의 다른 칸을 탐사하는 방식, 그리고 tombstone·clustering·적재율의 영향을 이해하기 위해 배운다. |
| 판단 근거 | 위 NIST hash table 자료는 open addressing을 table 내부의 다른 위치를 탐사하는 충돌 해결 계열로 구분한다. |
| 실제로 어디에 쓰이나 | entry를 배열 안에 두는 hash table 구현을 설계하거나, 체이닝과 메모리 배치·삭제·탐사 비용을 비교할 때 쓰인다. Java `HashMap`의 충돌 방식은 이것이 아니다. |
| 기억할 장면 | 지정 좌석이 차 있으면 같은 규칙으로 다음 빈 좌석을 찾는다. |
