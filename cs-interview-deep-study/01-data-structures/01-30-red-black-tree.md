# 1.30 Red-Black Tree: 레드-블랙 트리

태그: CS, 자료구조, RedBlackTree, TreeMap
...
Red-Black Tree는 노드에 색 정보를 두고 균형 조건을 유지하는 self-balancing binary search tree 계열이다. Java `TreeMap`은 Red-Black tree 기반 `NavigableMap` 구현체라고 Javadoc에 명시되어 있다.

### 쉬운 설명

트리가 너무 한쪽으로 기울지 않도록 노드에 빨강/검정 규칙을 붙여 자세를 잡는 트리다.
...
일반 BST는 입력 순서에 따라 한쪽으로 치우쳐 O(n)이 될 수 있다. Red-Black Tree는 높이가 과도하게 커지는 것을 막아 탐색/삽입/삭제를 O(log n) 계열로 유지한다.

---

## 3. 해결 범위와 비목표

이 문서가 직접 설명하는 범위는 Red-Black Tree: 레드-블랙 트리의 정의와 상위 자료구조에서의 역할이다. 정렬·동시성·영속성·특정 시간 복잡도는 별도 불변식이나 구현 계약이 있을 때만 보장된다.
...
Red-Black Tree: 레드-블랙 트리의 저장 표현은 상위 자료구조와 구현체에 따라 달라진다. 논리 관계와 실제 배열·Node·참조 표현을 구분한다.
...
새 node를 red로 넣으면 기존 경로의 black-height는 바로 바뀌지 않는다. 다만 parent도
red라면 규칙 4가 깨진다.

- uncle이 red: parent와 uncle을 black, grandparent를 red로 바꾸고 위로 전파한다.
- uncle이 black: 꺾인 모양이면 먼저 회전해 일자 모양으로 만들고, recolor와 반대
  방향 회전으로 red-red와 black-height를 복구한다.

마지막에는 root를 black으로 만든다.

### 삭제가 더 복잡한 이유

black node가 제거되면 한 경로의 black-height가 줄어 규칙 5가 깨질 수 있다. sibling의
색과 sibling child 색에 따라 recolor 또는 rotation을 수행해 부족한 black을 위로
전파하거나 해소한다. 면접에서는 모든 case를 암기하기보다 “어떤 불변식이 깨지고
회전·색 변경으로 무엇을 복구하는지”를 먼저 설명한다.

### Java TreeMap 연결

TreeMap은 Red-Black Tree 기반 `NavigableMap`이며 `containsKey`, `get`, `put`,
`remove`에 guaranteed O(log n)을 제공한다고 Javadoc에 명시한다. key 순서는 natural
ordering 또는 생성 시 comparator 기준이다.

### 삭제에서 확인하는 핵심

삭제 대상의 색보다 실제로 tree에서 빠져나가는 node의 색이 중요하다. red 제거는
black-height에 영향을 주지 않지만 black 제거는 한 경로의 black 수를 줄인다.

복구는 sibling과 sibling child의 색을 보고 다음 목적 중 하나를 수행한다.
...
모든 case 암기보다 어떤 불변식이 깨졌는지 먼저 설명해야 한다.
...
새 node는 일반적으로 red로 삽입한다. black으로 넣으면 해당 경로 black-height가 즉시
늘 수 있기 때문이다.
...
현재 red-red는 없어지지만 G의 parent도 red라면 위에서 다시 위반이 생기므로 G를
기준으로 반복한다. 마지막 root는 black으로 만든다.

#### parent red, uncle black, 일자 형태
...
G를 right rotation하고 P/G 색을 조정한다. 꺾인 LR/RL 형태라면 먼저 parent를 회전해
일자 형태로 만든 뒤 grandparent를 반대 방향으로 회전한다.
...
일반적으로 null child를 black NIL leaf로 간주하고 다음 규칙으로 설명한다.
...
이 규칙 때문에 가장 긴 root-leaf 경로가 가장 짧은 경로의 두 배를 넘지 않고,
node n개의 height가 O(log n)으로 제한된다.
...
TreeMap의 key 동일성은 natural ordering 또는 comparator의 비교 결과와 연결된다.
comparator가 0을 반환하는 두 key는 Map 관점에서 같은 key로 취급될 수 있다. 정렬
기준이 `equals()`와 일관되지 않으면 일반 Map 계약 관점에서 주의가 필요하다고
TreeMap Javadoc이 설명한다.
...
root에서 NIL leaf까지 모든 경로의 black node 수가 같고 red node가 연속될 수 없다.
따라서 가장 긴 경로는 black 사이에 red가 하나씩 들어가는 경우이며, red가 없는 가장
짧은 경로의 두 배를 넘지 않는다. 이 관계로 height가 O(log n)으로 제한된다.
...
완전 균형을 목표로 하기보다는 높이가 과도하게 커지지 않도록 제한하는 균형 트리다. AVL보다 균형 조건이 느슨한 편으로 설명한다.
...
삽입에서 parent와 uncle이 모두 red인 경우가 대표적이다. 둘을 black으로, grandparent를
red로 바꾸면 현재 위치의 red-red를 해소할 수 있다. 다만 grandparent 위에서 새
red-red가 생길 수 있어 위로 계속 확인한다.
...
red 삽입은 기존 NIL 경로의 black-height를 바로 바꾸지 않는다. 대신 parent도 red일
때 red-red 위반만 복구하면 된다.
...
표준 불변식에 root black이 포함된다. root를 black으로 바꾸는 것은 모든 root-leaf
경로에 black 하나를 동일하게 추가하므로 경로 간 black-height 동등성도 유지한다.
...
아니다. BST 정렬 관계를 가진 node 연결 구조다. inorder 순회 결과가 정렬되지만 메모리
배치나 level-order가 정렬 배열인 것은 아니다.
...
TreeMap이 Red-Black Tree 기반이고 Javadoc이 주요 연산의 guaranteed log(n) cost를
명시한다. 색 불변식이 tree height를 로그 규모로 제한한다.
