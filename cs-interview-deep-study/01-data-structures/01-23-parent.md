...
| 질문 | 먼저 잡을 답 |
|---|---|
| 왜 배우나 | 현재 node가 어느 subtree에 속하는지와 위쪽으로 어떻게 돌아갈지를 알아야 연결 변경과 경로 복원을 이해할 수 있다. |
| 판단 근거 | NIST는 parent를 child와 edge 하나로 직접 연결된 위 node로 정의한다. root만 parent가 없다. |
| 실제로 어디에 쓰이나 | heap에서 위로 값 올리기, 균형 tree 회전·복구, parent 정보를 저장한 탐색에서 출발점까지 경로를 되짚을 때 쓴다. 실제 parent 참조 저장 여부는 구현 선택이다. |
| 기억할 장면 | parent는 조상 전체가 아니라 바로 한 계단 위의 node다. |
